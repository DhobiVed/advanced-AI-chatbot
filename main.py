...
#     # Load environment variables
#     load_dotenv()
#     groq_api_key = os.getenv("GROQ_API_KEY")  # Ensure the env variable name matches
...
def chatbot_page1():
    ...
    # Sidebar for model selection
    model = st.sidebar.selectbox(
        'Choose a model', ["llama-3.1-8b-instant",
...
def chatbot_page2():
    ...
    st.markdown("""
        <div class='header-container'>
            <h1>🤖 Advanced PDF Q&A Chatbot</h1>
            <p>Upload PDF → Ask Questions → Get Answers & Summary</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    embedder, qa_model, summarizer = load_models()
    ...
    if st.session_state.pdf_text:
        ...
        if question:
            with st.spinner("Searching answer..."):
                ...
                context = " ".join([st.session_state.sentences[i] for i in top_idx])
    ...
    if st.session_state.pdf_text:
        ...
        if st.button("Generate Summary"):
            with st.spinner("Summarizing..."):
                text = st.session_state.pdf_text[:4000]  # SAFE LIMIT
                summary = summarizer(
                    text,
                    max_length=200,
                    min_length=80,
